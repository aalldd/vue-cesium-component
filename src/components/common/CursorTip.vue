<template>
  <div :class="classNames" :style="{left:floatLeft,top:floatTop}" ref="cursor">
    <slot></slot>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.container.appendChild(this.$refs.cursor);
    document.body.addEventListener('mousemove', this.mouseLisenter);
  },
  destroyed() {
    this.tipContainer && this.container && this.container.removeChild(this.tipContainer);
