        ...
        <template slot="custom" slot-scope="item">
          <div class="treeItem">
            ...
            <a-slider
              ...
              class="slider"
              @change="onLayerOpacityChange($event, item)"
            ></a-slider>
          </div>
        </template>
      </a-tree>
...
export default {
  ...
  mounted() {
    ...
    const resetChecked = () => {
      if (window.m3ds && window.commonConfig) {
        const layerGroupNamesTree =
          [window.commonConfig?.layerGroupNamesTree] || this.layerGroupTree;
        let count = 0;
        ...
        const checkedKeys = treeUtil
          .filter(treeData, (item) => item.visible === true)
          .map((n) => n.key);
  ...
  methods: {
    ...
    onLayerVisibleChange() {
      const itemFlatten = treeUtil
        .flatten(this.treeData)
        .filter((item) => this.checkedKeys.indexOf(item.key) >= 0)
        .map((ch) => {
          return ch.name;
        }); //找到对应的layerIndex
      if (this.m3ds?.length) {
        this.m3ds
          .filter((t, index) => itemFlatten.includes(this.findName(t)))
          .forEach((layer) => {
            layer.show = true;
          });
        this.m3ds
    ...
    changeLayerOpacity(value, item) {
      ...
      const tilesetList = this.m3ds.filter((t, index) =>
        itemFlatten.includes(this.findName(t))
      );
      tilesetList.forEach((tileset) => {
        if (value === 100) {
          ...
          tileset.style = new Cesium.Cesium3DTileStyle({
            color: {
              conditions: [["true", `color("white",${value / 100})`]],
            },
            show: true,
          });
        }
