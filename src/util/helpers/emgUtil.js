...
class emgUtil {
  ...
  positionTransfer = (position, transform) => {
    // 将经纬度坐标转为模型坐标
    let tileset = this.view.tilesetList.find(t => t.root) || this.view.tilesetList.find(t => t.layerId) ||  this.view.tilesetList[0];
  ...
  getInverseTransform = (tileset) => {
    let transform;
    let usetileset = tileset || this.view.tilesetList.find(t => t.root) || this.view.tilesetList.find(t => t.layerId) ||  this.view.tilesetList[0];
